...
`init` remembers everything it resolved for that repo (provider, models,
auth, limits, `include-*` flags) except the API token — see
[`configuration`](configuration.md). So a plain
`co-maintainer remake owner/repo --env=./.env` with no other flags reuses
all of it; only the token still needs to come from `--token` or the env
file.
