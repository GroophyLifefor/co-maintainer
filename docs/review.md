...
`CODEBASE.md` (the repository's own code layout, style, and test
conventions), fetches the PR description, comments, reviews, changed files,
and diff, then reports actionable findings — including a departure from the
codebase's own conventions even when no review-bar rule covers it. Use
