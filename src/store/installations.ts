...
export function markInstallationSuspended(
  ...
  getAppDb()
    .prepare(`UPDATE installations SET suspended_at = ? WHERE id = ?`)
    .run(suspended ? nowIso() : null, id);
}

export function markInstallationRemoved(id: number): void {
  getAppDb()
    .prepare(`UPDATE installations SET removed_at = ? WHERE id = ?`)
    .run(nowIso(), id);
}

export function getInstallation(id: number): InstallationRow | undefined {
  return getAppDb()
    .prepare<InstallationRow>(`SELECT * FROM installations WHERE id = ?`)
    .get(id);
}
