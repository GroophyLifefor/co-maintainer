...
test("Mermaid syntax tool documents every supported type", () => {
...
test("Mermaid syntax tool enforces the requested tool limit", () => {
  let failed = false;
  try {
    readMermaidSyntaxes({ tools: ["treeView-beta", "architecture-beta"] }, 1);
  } catch {
    failed = true;
  }
...
test("Mermaid tool schema exposes a bounded tools array", () => {
