...
import type { GitHubClient, Json, Options } from "../types.ts";
...
async function listPullRequestPages(
  ...
  if (canCatchUp) {
    ...
    if (reused > 0) {
      log(
        "fetch",
        `pull request listing · reused ${reused} cached PRs · ${selected.length} total`,
      );
    }
  }
...
> {
  ...
  log(
    "fetch",
    `codebase · ${important.length} files · concurrency=${concurrency}`,
  );
  const checkpoint = () => {
...
async function pullRequests(
  ...
  log(
    "fetch",
    `${selected.length} pull requests · concurrency=${options.ghConcurrent}`,
  );
