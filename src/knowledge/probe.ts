...
function isConventional(commit: Json): boolean {
  return /^(feat|fix|docs|refactor|test|build|ci|chore|perf|style)(\(.+\))?!?:\s/i.test(
    String((commit.commit as Json | undefined)?.message ?? ""),
  );
}

function commitStats(commits: Json[]): {
  mergeRatio: number;
...
function recommendCommitWindow(commits: Json[]): number | undefined {
  ...
  for (let start = 100; start < commits.length; start += 100) {
    ...
    const stable =
      Math.abs(stats.mergeRatio - recent.mergeRatio) <= 0.15 &&
...
function recommendPrWindow(
  ...
  const recentTwoYears = pulls.filter(
    (pull) => year(pull.updated_at) >= current - 1,
  ).length;
...
export function analyzeProbe(
  ...
  const diffs = detailSamples
    .map(
      (detail) => Number(detail.additions ?? 0) + Number(detail.deletions ?? 0),
    )
  ...
  const includeCommitHistory =
    usefulCommits >= 20 && commitSignal.mergeRatio < 0.75;
  ...
  if (maxPrMonths) {
    ...
    reasons.push(
      releaseAgeMonths <= 18
        ? `A release from ${latestReleaseAt.slice(
            0,
            10,
