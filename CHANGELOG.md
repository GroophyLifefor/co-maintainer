...
- **`sync` replaces `remake` (CORE-21).** The command that rebuilds a repository's guides now reads `sync` everywhere: help, logs, tips, the dashboard, and the docs. `remake` keeps working as a hidden alias with the same handler and flags, and `--remake-before-review` keeps working too. The stored values do not move: `job.type` stays `remake`, the per-repo key stays `remakeCron`, and the API route stays `/api/repos/:owner/:repo/remake`. `--sync-before-review` is the new name on the CLI, and `npm run sync` joins `npm run remake`.
- **`config` command (CORE-22).** `co-maintainer config list` prints every setting as key, value, and source (env, file, or default). `config get`, `config set`, `config unset`, and `config path [--all]` round it out. Secrets are masked: twelve characters or more show the last four after a row of dots, shorter ones show dots only, and the dashboard password hash is never listed. A value is checked against OpenRouter (`/key` and `/models`) before it is saved, and a bad key or model exits 2 without touching the file. `--ai-key` is an alias for `--token`, and `--review-blocking=model|severity` lives here.
- **`view` command (CORE-23).** Print the guides a repository has: `view owner/repo` prints all of them with a `== SKILL.md · built <date> ==` heading each, `view owner/repo review-guide` prints one raw so it can be redirected to a file, and `--list` and `--path` describe the guide folder. Without a repo argument it reads the git remote. A missing guide exits 2 with a `probe` hint.
- **`probe` estimates cost and time (CORE-24).** The plan now ends with an `estimate` block (jobs, token range, duration, dollars, and whether the basis is this repository's own history or the calibration set), a `next` block naming the recommended `init`, and `--run` to start that `init` in the same process. Pricing comes from OpenRouter and is cached for a day, and if it cannot be reached the dollar line is left out rather than guessed.
- **`--remote-host` and `--remote-token` (CORE-25).** Override the configured remote for one `review --remote` run without saving anything. Rejected with exit 2 outside `--remote` and in PR mode.
- **One cost and time line per run (CORE-26).** `init`, `sync`, local review, PR review, and remote review end with one line on stderr, for example `Done in 21.0s · 3,125 in, 1,308 out tokens · $0.0016`. `--json` stdout is untouched, and in a dashboard job the line goes to the job log instead of a terminal.
- **Every command rendered from one registry (CORE-20).** `help`, `<cmd> --help`, and `<cmd> -h` all exit 0, `serve --help` no longer starts a server, and a mistyped command or flag gets a did-you-mean within edit distance 2. `version` becomes a command alongside `--version`.
- **`GET /api/remote/guides?repo=owner/repo` (CORE-44).** A remote token can read a repository's guides, and `view --remote` uses it. The same bearer model as remote review applies: valid and active token, path validated, unknown or inactive repo answers 404.
- **Structured findings (CORE-40).** The model returns findings as JSON (`severity, blocking, path, lineFrom, lineTo, symbol, title, body, suggestion`) and co-maintainer renders the Markdown, so a boundary is never guessed. A schema and `tools` combination that the provider rejects is retried once without the schema, and unparseable JSON is retried once before falling back to the older Markdown reader. Comments posted before 0.5.0 still parse, so carry-over keeps working.
...
- **`remake` as a command name (CORE-21).** `co-maintainer remake` and `--remake-before-review` keep working, but they are hidden from help. Use `sync` and `--sync-before-review`. The stored names (`remakeCron`, `job.type="remake"`, `/api/repos/:owner/:repo/remake`) are not changing, so nothing needs migrating.
- **Old flat documentation addresses (CORE-80).** `co-maintainer.com/<page>.html` now redirects to `co-maintainer.com/docs/<page>.html`. The redirect stubs stay for now and are removed in a later release.

### Fixed

- **Windows crash on exit (CORE-11).** `process.exit` after a `fetch` left the undici connection pool open and tripped a libuv assertion (`0xC0000409`) on Windows. Every path now sets `process.exitCode` and lets the event loop drain, the heartbeat interval is `unref`ed and cleared on every error path, and the clone passes `-c core.longpaths=true`. The old code is proven red against the new test.
- **Failed AI units are not cached (CORE-30).** A unit whose output does not parse is validated, retried once, and, if it still fails, reported as skipped instead of being written to the cache, so the next run does not reuse garbage.
- **A skipped `PR_REVIEW_GUIDE.md` says why (CORE-31).** Below the threshold the run prints one line naming the reason instead of silently leaving the guide out.
- **The codegraph install prompt no longer hangs (CORE-50).** `readline` did not settle on EOF, so a non-interactive run waited forever on a question it could not answer.
...
[0.5.0-beta.1]: https://github.com/GroophyLifefor/co-maintainer/compare/v0.4.13...v0.5.0-beta.1
